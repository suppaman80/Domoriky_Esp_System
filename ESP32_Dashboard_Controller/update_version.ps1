...
switch ($Type.ToLower()) {
    "major" { 
        ...
        $minor = 0
        $patch = 0
        Write-Host "Major update selected"
    }
    "minor" { 
        $minor++
        $patch = 0
        Write-Host "Minor update selected"
    }
    "patch" { 
        $patch++
        Write-Host "Patch update selected"
    }
    default { 
        $patch++
        Write-Host "Defaulting to Patch update"
    }
}
...
"@
        ...
        Write-Host "Changelog aggiornato con: $Description"
    } else {
...
"@
        ...
        Write-Host "Creato nuovo file CHANGELOG.md"
    }
} else {
    Write-Host "Nessuna descrizione fornita. Changelog non aggiornato."
}
...
Write-Host "Versione incrementata a: $newVersion"
